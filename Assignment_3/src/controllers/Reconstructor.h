...
public:
	...
	struct Voxel
	{
		...
		int id = -1;							   // Based on the loc in the array
		std::vector<float> distanceCamera;		   // represent the distance for the voxel from each camera

		std::vector<float> distanceCamerasingle;

		std::vector<cv::Point3f> cornersVoxel;
		std::vector<std::vector<cv::Point>> cornes_vamera_projection;  // Projection corners
...
private:
	...
	std::vector<cv::Mat> shortest_voxel_Id;
	std::vector<cv::Mat> Shortest_Voxel_distance;
...
public:
	...
	const std::vector<cv::Mat> getShortestVoxelId() {
		return shortest_voxel_Id;
	}
	const std::vector<cv::Mat> getShortestVoxelDistance() {
		return Shortest_Voxel_distance;
	}
